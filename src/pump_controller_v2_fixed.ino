...
//  18. ★ FIX: App-stop blocks only the current schedule window,
//      not all future schedules. appManualStop replaced with
//      appManualStopUntil (unix timestamp of window end).
// ================================================================
...
#define EEPROM_ADDR_APP_MANUAL   8   // 1 byte  (kept for layout compat, now unused for scheduling)
#define EEPROM_ADDR_OT_TRIPPED   9   // 1 byte
// ★ NEW: store appManualStopUntil (4 bytes) at offset 10
...
uint32_t savedManualUntil = 0xFFFFFFFF; // ★ NEW dirty tracker
...
parse_schedule:
  ...
  if (newStart != sch1.startUnix || newStop != sch1.stopUnix) {
    ...
    sch1.active    = false;
    // ★ NEW: if the schedule time itself changed, also clear any stale block
    // so the new schedule window is not blocked by an old manual stop.
    if (appManualStopUntil > 0 && appManualStopUntil <= newStart) {
...
void checkSch(uint32_t nowUnix) {
  ...
  if (!sch1.active
      ...
      && sch1.sch_enabled
      && !(appManualStopUntil > 0 && nowUnix < appManualStopUntil)  // ★ CHANGED
      && !otTripped) {
...
bool checkButtonPress() {
  ...
  switch (btnState) {
    ...
    case BTN_HELD:
      if (!pinLow) {
        btnState = BTN_IDLE;
        if ((now - btnTime) < 3000) return true; // short press
      }
      break;
  }
  return false;
...
void saveEepromIfDirty() {
  ...
  if (appManualStopUntil != savedManualUntil) {
    ...
    savedManualUntil = appManualStopUntil; dirty = true;
  }
  if (dirty) { EEPROM.commit(); Serial.println(F("[EEPROM] Committed")); }
}

void loadEeprom() {
  uint32_t s, e, mu; uint8_t m, o;
  ...
  EEPROM.get(EEPROM_ADDR_MANUAL_UNTIL, mu);  // ★ NEW
  ...
  // ★ NEW: restore appManualStopUntil; if the stored timestamp is 0xFFFFFFFF
  // (uninitialized flash) treat it as 0 (no block).
  ...
  savedManual      = m;
  savedOtTrip      = o;
  savedManualUntil = appManualStopUntil;  // ★ NEW
